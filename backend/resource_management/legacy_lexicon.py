...
def save_editor(store, category_id, title, risk_label, entries, expected_version, description=None):
    with store._connect() as conn:
        ...
        body = LexiconContent(title=title, risk_label=risk_label or current.get('risk_label',''), entries=output, description=current.get('description','') if description is None else description)
