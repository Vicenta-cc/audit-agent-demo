...
新增 [统一运行入口](UNIFIED-RUNTIME.md)已实现 manifest/secrets 初始化、同源配置启动、强制 preflight 和服务模板。新环境优先按该文档执行；下文保留依赖准备、组件说明和手动开发参考，不再作为默认启动方式。旧 3398/8398 尚未迁移。
...
仓库已有 `.env.example`、Hermes 固定源版本与环境控制器。新增统一入口补上了配置和空数据库初始化，但还没有跨平台依赖完整锁定及自动安装。不能把“有配置文件”当成“已可重现部署”。
...
统一配置 schema、无密钥服务模板、空环境数据库初始化和公共 preflight 已由新入口实现。完整依赖锁定、自动依赖安装、云端操作系统的实际部署验收仍未完成，不能把它当成包含所有依赖的一键部署镜像。
