...
2026-09-11 完整环境增补：按用户要求仅导入 A/B，补齐本 worktree 独立数据库、完整 API、执行器及固定 3178 前端，安装本实验专属系统服务。A/B 真实问答通过；未借用 K2/3168 的账号或运行数据，新采集尚需独立登录并验收。当前启动、恢复和凭据存储说明以 [完整实验环境](M3_EXPERIMENT_ENVIRONMENT.md) 为准；下文临时端口、仅内存密钥等描述属于较早的局部验收方式。
