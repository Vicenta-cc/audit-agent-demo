...
中断或失败后，排查修复最多允许重试 1 次；重试前必须原子更新 runtime/retry-policy.json 的 retries_used。再次失败立即停止采集、卸载本次正式 worker、暂停定期检查并记录原因。抖音 verify/captcha 等需要人工验证时，直接记录并关闭任务及检查，不挂起等用户、不绕过验证、不自动恢复。保留前台、API、对话和已有内容供用户回来查看。该限制不把尚未发出任何采集请求的启动配置修复算作采集重试。
