...
`check` 检查 Hermes、提示词、密钥存在性、A/B 归档指纹和导入记录、运行文件及前端构建对应的源码指纹，不调用模型或采集。
...
**采集限制：**初始实验库没有已登录的抖音账号。用户需在本实验的“采集账号”入口单独登录后才能真实采集；不能直接使用 K2/3168 的会话或 cookie。执行器已启动，但本轮没有将“采集→审核→新报告”链路记作已验证。默认采集为每关键词 1 帖、分析上限 1 帖及低并发，限制保存在本实验配置中。
