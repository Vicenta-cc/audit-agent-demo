...
|---|---|---|---|---|---|
| `hermes_m0/account_activity_prompt.txt` | `62768caf1e59d4783c8c8ca362e30dd5bbee05f232898631bd7d31512a360c9e` | same | Final M1/M2.2 product System Prompt | runtime | every `AIAgent.run_conversation()` call as `system_message`; injected text hash after canonical `.strip()` is `c3e5d2d96d0beed0d803cefaf454d2e6c5f404474d1e022de895aaedc3cc78c1` |
...
The product prompt is loaded through package resources and guarded by its raw
file SHA-256. The held-out runner's `.strip()` behavior is then applied and the
result is supplied in the same `run_conversation(system_message=...)` position.
No legacy Planner prompt, concatenation, or rewritten prompt is used.
